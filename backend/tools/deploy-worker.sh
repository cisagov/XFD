...
PE_WORKER_TAG=pe-staging-worker
PW_WORKER_TAG=crossfeed-playwright-staging-worker
if [[ "$WORKER_TAG" == *integration* ]]; then
  PE_WORKER_TAG=pe-integration-worker
  PW_WORKER_TAG=crossfeed-playwright-integration-worker
fi
