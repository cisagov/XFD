# TODO
# POST/apiv1/iscore_was_vuln
# GET/apiv1/iscore_was_vuln/task/{task_id}
# POST/apiv1/iscore_was_vuln_prev
# GET/apiv1/iscore_was_vuln_prev/task/{task_id}
...
class TestIscoreWasVuln(unittest.TestCase):
    def test_online(self):
        try:
            api_url = "http://127.0.0.1:8089/apiv1/iscore_was_vuln"
        ...
        api_url = "http://127.0.0.1:8089/apiv1/iscore_was_vuln/task/" + task_id
        ...
        with open("results/iscore_was_vuln", 'r') as file:
...
class TestIscoreWasVulnPrev(unittest.TestCase):
    def test_online(self):
        try:
            api_url = "http://127.0.0.1:8089/apiv1/iscore_was_vuln_prev"
            response = requests.post(api_url, json=payload, headers=headers, verify=False)
            assert response.status_code == 200
            if response.status_code == 200:
                task_id = response.json()
        ...
        api_url = "http://127.0.0.1:8089/apiv1/iscore_was_vuln_prev/task/" + task_id
        response = requests.get(api_url, headers=headers,
                                verify=False)
        ...
        # Read the JSON data from the file
        with open("results/iscore_was_vuln_prev", 'r') as file:
            file_json = json.load(file)
