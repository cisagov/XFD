#!/bin/bash
# Start the in-container PE API, then run the SQS worker loop.
set -euo pipefail

export PE_API_URL="${PE_API_URL:-http://127.0.0.1:8000}"
export DJANGO_SETTINGS_MODULE=pe_reports_django.settings
...
cd /app
echo "Starting PE SQS worker (pe_worker.py)..."
exec python -u worker/pe_worker.py
