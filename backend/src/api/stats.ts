...
export const getVulnSummary = wrapHandler(async (event) => {
  ...
  const dnstwist_vuln_count = Number(dnstwist_results[0].count);
  ...
  const cred_breach_count = Number(cred_breach_result[0].count);

  const inventory_query = Domain.createQueryBuilder('domain')
    .leftJoinAndSelect(
