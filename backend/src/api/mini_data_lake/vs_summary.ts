...
export async function countVulnerableHosts(org_id: string): Promise<number> {
  ...
  const count = await hostRepository
    ...
    .where(
      'host.organizationId = :organizationId AND host.hostLive = :hostLive AND host.priority < :priority',
      {
  ...
  try {
    ...
    if (acronymList.includes(acronym)) {
      ...
      const mdl_org = await mdl_connection
        .getRepository(DL_Organization)
        .findOne({
          where: { acronym: acronym, retired: false },
          relations: ['cidrs', 'children', 'children.cidrs']
        });
      if (mdl_org) {
