...
import { DL_Organization, Cidr, Location , connectToDatalake } from '../../models';

export default async (organization: DL_Organization, cidrs: Cidr[],location: Location | null): Promise<string> => {
...
if (location !== null){
  ...
  locationId = ""
    ...
    const organization_promise = await DL_Organization.findOne({where:{id:orgId}, relations: ['cidrs']});
    if(organization_promise){
        const organization_entity: DL_Organization =organization_promise 
        if (location !== null){
          const location_promise = await Location.findOne({where:{id:locationId}})
          if(location_promise){
        ...
        const newCidrs = cidr_ids_list.filter(cidr => !organization_entity.cidrs?.some(item => item.id === cidr));
        organization_entity.cidrs?.push(...newCidrs.map(id => (plainToClass(Cidr,{ id }))))
