...
class Scheduler:
    ...
    async def launch_single_scan_task(self, organizations=None, scan=None, chunk_number=None, num_chunks=None, scan_task=None):
        ...
        command_options = scan_task.input or {
            'organizations': [{'name': org.name, 'id': str(org.id)} for org in organizations],
            'scanId': str(scan.id),
            'scanName': scan.name,
        ...
        try:
            if task_type == 'fargate':
                ...
                if not result.get('tasks'):
                    print(f"Failed to start Fargate task for scan {scan.name}, failures: {result.get('failures')}")
                    raise Exception(f"Failed to start Fargate task for scan {scan.name}")

                task_arn = result['tasks'][0]['taskArn']
                scan_task.fargateTaskArn = task_arn
                print(f'Successfully invoked scan {scan.name} with Fargate on {len(organizations)} organizations. Task ARN: {task_arn}')
            else:
    ...
    async def run(self):
        ...
        for scan in self.scans:
            ...
            if scan_schema.global_scan:
                ...
                organizations = get_scan_organizations(scan) if scan.isGranular else self.organizations
                orgs_to_launch = [org for org in organizations if self.should_run_scan(scan=scan, organization=org)]
    ...
    def should_run_scan(self, scan, organization=None):
        ...
        if last_finished_scan_task and last_finished_scan_task.finishedAt:
            ...
            frequency_seconds = scan.frequency * 1000  # Assuming frequency is in seconds.
...
async def handler(event):
    ...
    queued_scan_tasks = ScanTask.objects.filter(
        scanId__in=scan_ids,
        status='queued'
    ).order_by('queuedAt').select_related('scanId')
    ...
    await scheduler.initialize(
        ...
        orgs_per_scan_task=event.get('orgsPerScanTask') or int(os.getenv('SCHEDULER_ORGS_PER_SCANTASK', '1'))
    )
