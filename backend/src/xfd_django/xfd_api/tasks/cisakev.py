...
class CisaKevScan:
    ...
    def run(self):
        ...
        try:
            ...
            self.logger.info("Retrieved %s KEV entries.", len(vulnerabilities))
            ...
            self.logger.info("Inserted %s records into CisaKevCatalog.", len(entries))

            # Step 3: Flag CVEs that are now on the KEV list
            flagged_count = flag_known_exploited_cves()
            self.logger.info(
                "Flagged %s CVE records as known exploited.", flagged_count
            )
