...
def extract_ssvc(adp_items: Optional[List[Dict[str, Any]]]) -> Dict[str, Optional[str]]:
    ...
    Kept for sources (e.g. the old Redshift AE feed) that still nest SSVC
    decision points inside an ADP container. cyhy_cve_data's Databricks rows
    expose exploitation/automatable/technical_impact directly on the row, so
    callers reading from that table don't need this helper.
    """
...
def extract_ssvc_from_cvssv4(metrics_list) -> Dict[str, Optional[str]]:
    """
    Best-effort SSVC decision points derived from a CVSS v4 supplemental-metrics block.

    cyhy_cve_data has no separate ADP/SSVC container - CRASM-4102 asked to use
    CVSS v4's supplemental/threat metrics as a stand-in for now, since a few
    of them name-match SSVC decision points directly:
      - automatable       <- cvssV4_0.Automatable    (same concept, same name)
      - value_density     <- cvssV4_0.valueDensity   (same concept, same name)
      - exploitation      <- cvssV4_0.exploitMaturity (closest available proxy -
                             CVSS v4's Exploit Maturity threat metric, not a
                             true SSVC Exploitation value/vocabulary)
      - mission_wellbeing <- cvssV4_0.Safety          (closest available proxy -
                             CVSS v4's Safety supplemental metric, not a true
                             SSVC Mission & Well-being value/vocabulary)

    technical_impact has no CVSS v4 supplemental-metric equivalent, so it's
    always returned as None here - callers should keep sourcing it elsewhere.

    `metrics_list` is a CNA metrics array (e.g. cna["metrics"]); this returns
    values from the first entry that has a populated cvssV4_0 block, since a
    CVE can have several metrics entries (different CVSS versions/sources)
    and only some may include v4. Values are returned as-is (not lowercased);
    callers decide normalization.
    """
    result: Dict[str, Optional[str]] = {
        "exploitation": None,
        "automatable": None,
        "technical_impact": None,
        "mission_wellbeing": None,
        "value_density": None,
    }
    if not isinstance(metrics_list, list):
        return result
    ...
    for metric in metrics_list:
        if not isinstance(metric, dict):
            continue
        v4 = metric.get("cvssV4_0")
        if isinstance(v4, dict) and v4:
            result["exploitation"] = v4.get("exploitMaturity")
            result["automatable"] = v4.get("Automatable")
            result["mission_wellbeing"] = v4.get("Safety")
            result["value_density"] = v4.get("valueDensity")
            return result
...
def extract_weaknesses_from_problem_types(problem_types_json) -> list[str]:
    """
    From cna.problemTypes (list of objects), gather CWE IDs (or descriptions).
    ...
    cyhy_cve_data already provides a pre-computed top-level `weaknesses`
    array, so this is only a fallback for when that's empty.
    """
    out: list[str] = []
