...
| Daily report tracker persistence | `was-update-tracker` | Started | The WAS-owned command injects an environment-backed Qualys adapter before running the bundled tracker flow, so production tracker execution does not read or create `was_config.txt`. Tracker rows are written to Postgres, optional `--tag` scopes scan retrieval and database writes after schedule discovery, and Qualys webapp deletion requires `--delete-apps`. |
