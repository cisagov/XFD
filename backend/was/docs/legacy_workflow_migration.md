...
| Orchestrate production report pipeline | `generate_full` and active `main` path | `was_reports.reporting.report_service` | Active | Connects managed Qualys retrieval, CSV transformation, metrics, charts, attachments, finding ages, template assembly, LaTeX rendering, and atomic password encryption. Each run uses a private production-resource copy. Qualys report polling continues until `COMPLETE` by default, emits periodic progress, and persists active report IDs for restart recovery. Postgres schedule claims prevent duplicate container runs, while the local lock protects writes within one output filesystem. Report comparison remains available while equivalence review continues. |
| Delete temporary Qualys report | `delete_report` | `was_reports.reporting.report_retrieval` | Active | Context-managed retrieval guarantees cleanup after downstream processing and clears the deleted XML report ID from the active database run. |
