...
def https_check(endpoint):
    ...
    try:
        hostname = endpoint.url[8:]  # Strip 'https://'
        server_location = ServerNetworkLocation(hostname=hostname, port=443)
        commands = [ScanCommand.CERTIFICATE_INFO]
        ...
        if CA_FILE is not None:
            scan_kwargs["scan_commands_extra_arguments"] = {
                ScanCommand.CERTIFICATE_INFO: CertificateInfoExtraArgument(
                    custom_ca_file=Path(CA_FILE)
                )
            }

        scan_request = ServerScanRequest(**scan_kwargs)
        scanner = Scanner()
        ...
        cert_plugin_result: CertificateInfoScanResult = cert_plugin_attempt.result

        endpoint.live = True
        ip = scan_result.server_location.ip_address
        if endpoint.ip is None:
            endpoint.ip = ip
        elif endpoint.ip != ip:
            utils.debug(
    ...
    try:
        endpoint.https_cert_chain_len = sum(
            len(dep.received_certificate_chain)
            for dep in cert_plugin_result.certificate_deployments
        )

        endpoint.https_missing_intermediate_cert = False
        if not endpoint.https_self_signed_cert and endpoint.https_cert_chain_len < 2:
            endpoint.https_missing_intermediate_cert = True
            all_verified = all(
                dep.verified_certificate_chain is not None
                for dep in cert_plugin_result.certificate_deployments
            )
            if not all_verified:
            ...
            elif custom_trust is True and public_trust is False and PT_INT_CA_FILE:
                try:
                    scanner = Scanner()
                    ...
                    if cert_plugin_attempt.result is not None:
                        cert_plugin_result = cert_plugin_attempt.result
                        if all(
                            dep.verified_certificate_chain is not None
                            for dep in cert_plugin_result.certificate_deployments
                        ):
