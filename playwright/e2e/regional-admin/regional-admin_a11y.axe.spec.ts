...
test.describe('A11y — Regional Admin (axe, minimal critical surfaces)', () => {
  ...
  // Regional Admin Dashboard / User Registration
  test('Regional Admin Dashboard (User Registration): no serious/critical violations', async ({
    pageAsRegionalAdmin,
    makeAxeBuilder
  }, ti) => {
    const page = pageAsRegionalAdmin;
    await page.goto(ROUTES.REGION_ADMIN_DASHBOARD);

    // Pick a heading that actually exists here — adjust if your page title differs
    const heading = page
      .getByRole('heading', { name: /Regional Admin dashboard/i })
